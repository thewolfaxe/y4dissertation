...
function sysCall_threadmain()
    -- Initialization phase, need to get all the usful handles
    jh={-1,-1,-1,-1,-1,-1}
    for i=1,6,1 do
    ...
    while true do
        while true do
            ...
            -- finding a previous valid state
            local breaker = false
            if c == nil then
            ...
            -- this state was successful, saving it
            ...
            print("Attempting to generate a reverse path...")
            ...
            if goback then
                sim.setJointTargetPosition(railJoint, 0)
                moveDistance = 2
                goback = false
            else
                sim.setJointTargetPosition(railJoint, sim.getJointPosition(railJoint) + moveDistance)
            end
            
            sim.wait(moveDistance/maxVel)
        end
    end
end